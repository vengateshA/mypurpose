...
const BluetoothExample = () => {
  ...
  const writeCmd = (cmd) => {
    if (!port) {
      console.error('[writeCmd] Port not available.');
      return;
    }
  
    const writer = port.writable.getWriter();
    const encoder = new TextEncoder();
    const cmdArrayBuffer = encoder.encode(cmd + '\r');
    
    console.log('[SEND]', cmd);
    writer.write(cmdArrayBuffer);
    writer.releaseLock();
  };
...
//   try {
//     await outputStream.write(cmd + '\r');
//   } catch (error) {
//     console.error('[writeCmd] Error:', error);
//   }
// };
